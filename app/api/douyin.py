from fastapi import APIRouter, Request
from fastapi.responses import JSONResponse

from app.douyinsearch.errors import DouyinSearchError
from app.douyinsearch.schemas import SearchRequest
from app.douyinsearch.service import douyin_service

router = APIRouter()


def error_response(error: DouyinSearchError):
    status_code = 404 if error.code == "RESULT_EXPIRED" else 400
    return JSONResponse(
        status_code=status_code,
        content={"success": False, "error": error.to_payload()},
    )


@router.get("/health")
async def health():
    return douyin_service.health()


@router.post("/session/check")
async def session_check():
    return await douyin_service.check_session()


@router.post("/search")
async def search(request: SearchRequest):
    try:
        return await douyin_service.search(request)
    except DouyinSearchError as error:
        return error_response(error)


@router.get("/results/{result_id}")
async def get_result(result_id: str):
    try:
        return douyin_service.get_result(result_id)
    except DouyinSearchError as error:
        return error_response(error)


@router.get("/results/{result_id}/cover")
async def cover(result_id: str):
    try:
        return await douyin_service.proxy_cover(result_id)
    except DouyinSearchError as error:
        return error_response(error)


@router.get("/results/{result_id}/stream")
async def stream(result_id: str, request: Request):
    try:
        return await douyin_service.proxy_stream(result_id, request.headers.get("range"))
    except DouyinSearchError as error:
        return error_response(error)
